...
config :viztube_web, ViztubeWeb.Endpoint,
  # url: [host: "localhost"],
  secret_key_base: {:system, "SECRET_KEY_BASE"} || "${SECRET_KEY_BASE}",
...
config :viztube_web, ViztubeWeb.Mailer,
  ...
  server: {:system, "SMTP_HOSTNAME"} || "${SMTP_HOSTNAME}",
  hostname: {:system, "SMTP_HOSTNAME"} || "${SMTP_HOSTNAME}",
  port: 465,
  username: {:system, "SMTP_USERNAME"} || "${SMTP_USERNAME}",
  password: {:system, "SMTP_PASSWORD"} || "${SMTP_PASSWORD}",
